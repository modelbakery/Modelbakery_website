...
t.test(monetary~dup, data = cust_dup_tbl)
t.test(monetary~dup, data = sample_dup_tbl)
```

### 3.3 Visualisation on same-day repeated and non-repeated customer 

The visualisation here provides more insight. Customer monetary value, by nature, has two noticeable binary distributions (there can always be more) overlapped from one and another. It may be valid to make a rather weak assumption that there is a threshold from its low-density region to high-density region in its monetary values in scale. 
...
Perhaps it is the __accessibility__ upon purchase act as a confounder between monetary value and same-day purchase customer behaviour. 

```{r}

P3_C_dup <- cust_dup_tbl %>% ggplot(aes(x = dup, y = monetary, fill = dup)) +
  geom_col(alpha = 0.5) +
  scale_y_continuous(labels = scales::dollar_format()) +
  scale_fill_tq() +
  theme_tq()

P3_B_dup <- cust_dup_tbl %>% ggplot(aes(x = dup, y = monetary, fill = dup)) +
  geom_boxplot(alpha = 0.5) +
  scale_y_continuous(labels = scales::dollar_format()) +
  scale_fill_tq() +
  theme_tq()

P3_V_dup <- cust_dup_tbl %>% ggplot(aes(x = dup, y = monetary, fill = dup)) +
...
Thorough analysis on their purchasing pattern can add valuable information for cross-sell, offer, pricing strategy. However, this is not my primary focus for now. I like to make quick, actionable marketing decisions on 'target' customers.
