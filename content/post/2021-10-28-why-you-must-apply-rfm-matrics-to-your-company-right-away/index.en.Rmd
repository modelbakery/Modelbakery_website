...
# DT::datatable(trans_df, class = 'cell=border stripe', rownames = F, filter = 'top',
...
#                 scrollX = TRUE))
# 
# DT::datatable(resp_df, class = 'cell=border stripe', rownames = F, filter = 'top',
#               editable = TRUE, extensions = 'Buttons', options = list(
#                 pageLength = 5,
#                 dom = 'Bfrtip',
#                 width = '55%',
...
more random words ^[Analytic perspective notes]

Analytic perspective notes 
...
dup_dt <- rbind(y_dup_df, n_dup_df) %>% 
    dplyr::group_by(dup) %>% 
    dplyr::summarise(count = n(),
                     med_pur_amount = median(amount) %>% format_dollar,
                     avg_pur_amount = mean(amount)   %>% format_dollar) 
  
dup_dt %>%
    set_names(c("Same Day Purchase", "No. Customer", "Median Purchase", "Average Purchase")) %>%
    kbl() %>% 
    kable_material_dark()
...
std_mean <- 12.0533540952864
std_sd   <- 0.12791922963096
```

```{r}
mon_sd_tbl %>% 
  plot_time_series(month, revenue, .interactive = FALSE)
```
