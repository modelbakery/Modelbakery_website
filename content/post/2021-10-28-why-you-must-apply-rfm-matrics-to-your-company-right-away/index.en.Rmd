...
Clustering these groups exceeds my goal in this post hence I would not explore more than illustrating some of the classical examples from below. 
...
```{r}
potential_new_cust <- rfm_score_df %>%
  select(-c(recency_days:monetary, RFM_Score_norm)) %>% 
  filter(M_score >= 2 & F_score == 1 & R_score >= 5) %>% 
  arrange(desc(RFM_Score)); crosssell_cust %>% head(10) %>% kbl() %>% kable_material_dark()
```
