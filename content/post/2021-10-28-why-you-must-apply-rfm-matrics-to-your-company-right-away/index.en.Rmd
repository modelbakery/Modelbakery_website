...
* Comparative analysis and potential aspects on "what is the behind the scene" of customers at low and high monetary regions are needed. 
...
* Inquire customer information on geographical and payment method from database in order to approach on defining service "accessibility" and its scoring board. 
...
The volatility of daily and weekly transcation data can bury many useful information such as trend or activity variance. The moving average smoothing (red) removes fine-grained variation between the time steps by averaging values within given window size which slides along the time series. 
...
Now the monthly revenue time series reveals some useful components. 
* 
...
mon_sd_tbl %>% 
  ggplot(aes(x = month, y = (revenue_sd))) +
