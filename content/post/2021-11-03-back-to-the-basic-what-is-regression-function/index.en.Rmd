...
Understanding that skewness and scale are different is important, it can be easily observed by t-statistics and correlation coefficients. It can be seem non-ordinary to split the numeric target variable into two subgroups during the data interpretation. However, as seen above, it adds more visual interpretation then only having to look at correlation. I believe this approach can also be powerful in recongising hierachical relationships between the variable X's. 
...
tidy(lmfit_00tr); glance(lmfit_00tr)

vip(lmfit_00tr)

predict(lmfit_00tr, new_data = Boston_te)
...
ggplot(data = Boston_housing_results_lm00,
       mapping = aes(x = .pred, y = lmedv)) +
