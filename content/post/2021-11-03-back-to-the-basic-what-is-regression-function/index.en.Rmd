...
"Will my model actually be used?"- the one of the first questions as analyst or researcher must ask them selevs. - by Frank E from the book: Regression Modeling Strategies with Applications to Linear Models, Logistics Regression, and Survial Analysis (pg 4). One of its reasons it stated were the potential users of the model did not trust the relationship, weights, or variables used to make the prediction. 

The objective of study here is pre-processing, understanding variables, powerful visulaisation in a workflow mechanism to guide linearlising the model with using Boston Housing Dataset. The design of this workflow includes variable transformation (asymmetric reduction), PCP and correlation matrix upon target variable subgroup projection, Cook's Distance outlier dignostic and treatment, and lastly linear regression output (rmse: 0.151, r-squared: 0.856). 
...
I believe that the more complicated the problem is, the more important its interpretation becomes. In other words, we should always consider the possibility of simpler way to approach any form of problems. 
...
# Backward and both gives the same output 
...
# Stepwise Selection method: VIP visualisation 
...
tidy(lmfit_00tr) %>% kbl %>% kable_material()
glance(lmfit_00tr) %>% kbl %>% kable_material()
...
```{r}
Boston_housing_results_lm00 <- predict(lmfit_00tr, new_data = Boston_te) %>%
  bind_cols(Boston_te) %>% 
  mutate(residue = lmedv - .pred) %>% 
  select(.pred, residue, lmedv, everything())
```
...
rmse(Boston_housing_results_lm00, 
     truth = lmedv,
     estimate = .pred) %>% kbl %>% kable_material()

rsq(Boston_housing_results_lm00,
    truth = lmedv,
    estimate = .pred) %>% kbl %>% kable_material()

Boston_housing_results_lm00 %>% 
...
Boston_housing_results_lm00 %>% 
  ...
  geom_point(color = '#006EA1') + 
  geom_smooth(method = "lm", se = FALSE, color = 'orange'); mean(Boston_housing_results_lm00$residue)
```
...
```{r}
cof <- round(lmfit_00tr$fit$coefficients, 3) 
n <- names(cof)
