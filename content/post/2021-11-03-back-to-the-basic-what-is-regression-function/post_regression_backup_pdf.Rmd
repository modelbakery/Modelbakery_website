...
## Outlier Detection: On Fluential Observation by Cooks Distance 
...
### Outlier Treatment #1: Predictor Varaibles IQR-outliers mean treatment at influential Observation detected 
...
# Remove the Target Variable Extreme Outliers 
...
Boston_outlier_tbl <- Boston_transform %>% 
  ...
  mutate(row_id = row_number(),
         cooksd_flag = case_when(row_id %in% influential ~ 1,
                                 TRUE ~0)) %>% 
    mutate_all(funs(x = impute_rf(. ~ .))) 


Boston_outlier_tbl %>% 
  filter(cooksd_flag == 1) %>% 
  mutate_at(vars(lcrim_x:srlstat_x), ~replace(., . == 1, NA)) %>% 
  mutate_all(funs(x = impute_rf(. ~ .))) 
...
Boston_outlier_tbl %>% 
  filter(cooksd_flag == 1) %>% 
  select(matches(c("id", "_x"))) %>% 
  summarise_all(sum)


Boston_outlier_tbl %>% 
  select(outlier_flag:outlier_treat_w_IQR) %>% 
  group_by(outlier_flag) %>% 
  
  summarise_all(funs(x = sum(.))) 
Boston_outlier_tbl %>% 
  filter(outlier_flag == 2) %>% 
  pull(row_id) 


  mutate_at(vars(lcrim:srlstat), ~outlier_treatment(.)) 
```
...
Boston %>% 
  ...
  ggplot(aes(medv, rm)) +
  geom_point()
outlier_tbl <- Boston_transform[influential, ]
colnames(outlier_tbl)
  ggplot(aes())
...
for(i in 1:length(var_names)){
  univar <- Boston_transform[,i]
  outlier_values<- boxplot.stats(univar)$out
  outier <- which(univar %in% outlier_values)
  outier <- c(outier, out_rm) 
  out <- outier[-which(duplicated(outier))]
}
out

outlier_values <- boxplot.stats(Boston_transform$lcrim)$out 
which(Boston_transform$lcrim %in% outlier_values)
which(Boston_transform$lcrim %in% outlier_values)
...
```{r}
boxplot.stats(Boston_transform$lcrim)[4]
outlier_values <- boxplot.stats(Boston_transform$lcrim)$out  # outlier values.
boxplot(Boston_transform$lcrim, main="lcrim", boxwex=0.1) +
mtext(paste("Outliers: ", paste(outlier_values, collapse=", ")), cex=0.6)

boxplot(lmedv ~ lcrim, data=Boston_transform, main="Boxplot for Pressure height (continuos var) vs Ozone")
...
## Variable Selection: Best Subsets Selection 
...
```{r}
plot(regsubfit_full, scale = "r2")
plot(regsubfit_full, scale = "bic")
```
...
summary_relationship_tbl %>% 
  filter(str_detect(label, pattern = "chas"))
